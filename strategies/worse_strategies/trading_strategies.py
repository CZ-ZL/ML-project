...
class TradingStrategy():
    ...
    def calculate_profit_for_signals(self, curr_ratio, next_ratio):
        """Calculate profit with spread consideration"""
        ...
        for direction in ['buy_currency_a', 'sell_currency_a']:
            if direction == 'buy_currency_a':
                # Pay ask price to enter, receive bid price to exit
                entry_price = curr_ratio + spread/2
                exit_price = next_ratio - spread/2
                profit = self.fixed_position_size * (exit_price - entry_price) / exit_price
            else:
                # Receive bid price to enter, pay ask price to exit
                entry_price = curr_ratio - spread/2
                exit_price = next_ratio + spread/2
                profit = self.fixed_position_size * (entry_price - exit_price) / exit_price
    ...
    def calculate_profit(self, strategy_name, trade_direction, curr_ratio, f_i, use_kelly):
        """Execute trades with spread-adjusted prices"""
        profit = 0
        
        # Close existing position with spread
        if self.open_positions[strategy_name]['type'] is not None:
            profit += self.close_position(strategy_name, curr_ratio)
        ...
        if trade_direction != 'no_trade':
            ...
            if trade_direction == 'buy_currency_a':
                # Pay ask price
                adjusted_ratio = curr_ratio + spread/2
                bet_size_a = min(base_bet_size_a, self.wallet_a[strategy_name])
                bet_size_b = bet_size_a * adjusted_ratio
                ...
                if bet_size_b <= self.wallet_b[strategy_name]:
                    ...
                    self.open_positions[strategy_name] = {
                        ...
                        'size_a': bet_size_a,
                        'size_b': bet_size_b,
                        'entry_ratio': adjusted_ratio  # Store entry with spread
                    }
    ...
    def close_position(self, strategy_name, curr_ratio):
        """Close positions with spread consideration"""
        ...
        if position['type'] == 'long':
            # Sell at bid price
            exit_ratio = curr_ratio - spread/2
            self.wallet_a[strategy_name] -= position['size_a']
            self.wallet_b[strategy_name] += position['size_a'] * exit_ratio
            profit = position['size_a'] * (exit_ratio - position['entry_ratio']) / exit_ratio
            
        elif position['type'] == 'short':
            # Buy back at ask price
            exit_ratio = curr_ratio + spread/2
            required_b = position['size_a'] * exit_ratio
            if self.wallet_b[strategy_name] >= required_b:
                self.wallet_b[strategy_name] -= required_b
                self.wallet_a[strategy_name] += position['size_a']
                profit = position['size_a'] * (position['entry_ratio'] - exit_ratio) / exit_ratio
